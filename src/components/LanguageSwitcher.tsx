...
import { supportedLocales, type Locale } from '../siteConfig';
...
export function LanguageSwitcher() {
  ...
  function choose(l: Locale) { setLocale(l); setOpen(false); }
  ...
  return (
    <div className={`language-switcher${open ? ' open' : ''}`} ref={wrapRef}>
      <button type="button" className="globe-trigger" aria-haspopup="listbox" aria-expanded={open} aria-label="Change language" onClick={toggle} onKeyDown={onTriggerKey}>
      ...
      {open && (
        <ul className="language-menu" role="listbox" aria-label="Select language" ref={listRef} onKeyDown={onListKey}>
          {supportedLocales.map(l => (
            <li key={l}>
              <button type="button" role="option" aria-selected={l === locale} data-active={l === locale || undefined} data-locale={l} className="locale-item" onClick={() => choose(l)}>{l.toUpperCase()}</button>
            </li>
          ))}
        </ul>
      )}
